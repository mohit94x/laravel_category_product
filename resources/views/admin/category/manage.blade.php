...
@section('content')
    <div class="container">
        ...
        <div class="w-50 mx-auto">
            @if ($aRow)
                <form action="{{ route('category.update', $aRow->id) }}" method="post">
                    @method('patch')
                @else
            ...
            @csrf
            <fieldset>
                <legend class="text-center"> {{ $aRow ? 'Update' . $aRow->name : 'Create New' }} Categoty</legend>
                <div class="form-group">
                    <label for="exampleInputEmail1">Selete Parent Category</label>
                    <select name="parent_id" id="" class="form-control">
                        <option value="">Select Parent Category</option>
                        @foreach ($category as $c_item)
                            <option {{ ($aRow && $c_item->id == $aRow->parent_id) ? 'selected' : '' }} value="{{ $c_item->id }}">{{ $c_item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
